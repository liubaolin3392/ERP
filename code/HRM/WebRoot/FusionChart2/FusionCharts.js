 ...
 * 
 * Third-party attributions: SWFObject v2.2
 * <http://code.google.com/p/swfobject/> JSON v2 <http://www.JSON.org/js.html>
 * Firebug Lite 1.3.0 <http://getfirebug.com/firebuglite> jQuery 1.4.2
 * <http://jquery.com/>
 */
(function() {
	...
	f.core = function(a) {
		...
		if (f.core.items[this.id] instanceof f.core) {
			this.id = f.uniqueId();
			f.raiseWarning(this, '06091847', 'param', '',
...
(function() {
	...
	var j = {
		...
		addListener : function(a, b, c) {
			...
			if (typeof a !== 'string') {
				f.raiseError(this, '03091549', 'param',
						'::EventTarget.addListener',
						new Error('Unspecified Event Type'));
				return
			}
			if (typeof b !== 'function') {
				f.raiseError(this, '03091550', 'param',
						'::EventTarget.addListener',
		...
		removeListener : function(a, b, c) {
			...
			if (typeof a !== 'string') {
				f.raiseError(this, '03091559', 'param',
						'::EventTarget.removeListener',
						new Error('Unspecified Event Type'));
				return
			}
			if (typeof b !== 'function') {
				f.raiseError(this, '03091560', 'param',
						'::EventTarget.removeListener',
						new Error('Invalid Event Listener'));
				return
			}
		...
		triggerEvent : function(a, b, c) {
			if (typeof a !== 'string') {
				f.raiseError(this, '03091602', 'param',
		...
		debugMode : {
			outputFormat : function(a) {
				if (a
						...
						&& typeof m.outputHelpers[a = a.toLowerCase()] === 'function') {
			...
			enabled : function(a, b, c) {
				...
				if (typeof a === 'boolean' && a !== m.enabled) {
					i.core[(m.enabled = a)
							? 'addEventListener'
							: 'removeEventListener']('*', m.outputHandler)
				}
				if (typeof b === 'function') {
					m.outputTo = b
				}
			...
			_enableFirebugLite : function(a) {
				...
				b.src = typeof a === 'string'
						? a
						: i.core.options.scriptBaseUri + 'firebug-lite.js';
...
(function() {
	...
	var g = function() {
		f.raiseError(this, '25081845', 'run', '::RendererManager',
	...
	f.renderer = {
		register : function(a, b) {
			...
			if (h[a] !== undefined) {
				f.raiseError(f.core, '03091438', 'param',
		...
		setDefault : function(a) {
			if (!a || typeof a.toString !== 'function') {
				f.raiseError(f.core, '25081731', 'param',
						'::RendererManager>setDefault',
						'Invalid renderer name specified in "name"');
				return false
			}
			if (h[a = a.toString().toLowerCase()] === undefined) {
				f.raiseError(f.core, '25081733', 'range',
						'::RendererManager>setDefault',
	...
	var j = function(a) {
		return function() {
			if (this.ref === undefined || this.ref === null
					|| typeof this.ref[a] !== 'function') {
				f
						.raiseError(this, '25081617', 'run', '~' + a + '()',
	...
	f.addEventListener('BeforeInitialize', function(a) {
		var b = a.sender;
		if (typeof b.options.renderer === 'string'
				&& h[b.options.renderer.toLowerCase()] === undefined) {
		...
		if (store[b.id].initialized !== true
				&& typeof store[b.id].init === 'function') {
			store[b.id].init();
	...
	f.addEventListener('Loaded', function(e) {
		...
		if (chartObj === undefined || chartObj === null
				|| typeof chartObj.getExternalInterfaceMethods !== 'function') {
			return
		}
	...
	f.extend({
		render : function(a) {
			...
			if (a === undefined || a === null) {
				f.raiseError(this, '03091456', 'run', '.render()',
			...
			if (k(this.id, a)) {
				f
						.raiseError(
								...
								'run',
								'.render()',
								new Error("A duplicate object already exists with the specific Id: "
										+ this.id));
				return this
			}
	...
	f.extend({
		setCurrentRenderer : f.renderer.setDefault,
		render : function() {
			var a = ['swfUrl', 'id', 'width', 'height', 'renderAt',
...
(function() {
	...
	var i = function(a) {
		if (xStore[a] && typeof xStore[a].abort === 'function'
				&& xStore[a].readyState && xStore[a].readyState !== 0) {
			xStore[a].abort();
			return true
		}
		return false
	...
	g.addDataHandler = function(b, c) {
		if (typeof b !== 'string' || h[b.toLowerCase()] !== undefined) {
			g.raiseError(g.core, '03091606', 'param',
					'::DataManager.addDataHandler',
	...
	g.addEventListener('BeforeInitialize', function(a) {
		...
		dataStore[b.id] = '';
		cache[b.id] = {};
		if (b.options.dataSource !== undefined
				&& typeof b.options.dataFormat === 'string') {
	...
	g.extend({
		setChartDataUrl : function(c, d, e) {
			if (d === undefined || d === null
				...
				g.raiseError(g.core, '03091609', 'param', '.setChartDataUrl',
			...
			xStore[this.id] = g.ajax.get(decodeURIComponent(c), function(a, x) {
				...
				if (b !== true) {
					...
					g.raiseEvent('DataLoadCancelled', {
						source : 'XmlHttpRequest',
						url : c,
						dataFormat : f,
						xmlHttpRequestObject : x
		...
		setChartData : function(a, b, c) {
			if (b === undefined || b === null
				...
				g.raiseError(g.core, '03091610', 'param', '.setChartData',
						new Error('Invalid Data Format'))
			}
			...
			if (typeof e === 'undefined') {
				g.raiseError(g.core, '03091611', 'param', '.setChartData',
			...
			if (c !== true) {
				if (this.options.safeMode === true
					...
					g
							.raiseWarning(this, '23091255', 'run',
									'::DataHandler~update',
		...
		getChartData : function(a, b) {
			var c, parseResult;
			if (a === undefined || typeof a.toString !== 'function'
					|| (c = h[a = a.toString().toLowerCase()]) === undefined) {
				g
						.raiseError(
								this,
								'25081543',
			...
			parseResult = (typeof cache[this.id][a] === 'object')
					? cache[this.id][a]
					: cache[this.id][a] = c.decode(dataStore[this.id], this);
			return Boolean(b) === true ? parseResult : parseResult.data
		}
	}, true);
	g.extend({
		transcodeData : function(a, b, c, d) {
			if (!b || typeof b.toString !== 'function' || !c
					|| typeof c.toString !== 'function'
					|| h[(c = c.toString().toLowerCase())] === undefined
					|| h[(b = b.toString().toLowerCase())] === undefined) {
				g
						.raiseError(
								this,
								'14090217',
...
(function() {
	...
	f.extend({
		...
		setDataXML : function(a) {
			if (a === undefined || a === null
				...
				f.raiseError(this, '25081627', 'param', '~setDataXML',
		...
		setDataURL : function(a) {
			if (a === undefined || a === null
					|| typeof a.toString !== 'function') {
				f.raiseError(this, '25081724', 'param', '~setDataURL',
		...
		isActive : function() {
			if (!this.ref || document.getElementById(this.id) !== this.ref
					|| typeof this.ref.signature !== 'function') {
				return false
			}
			try {
	...
	window.getChartFromId = function(a) {
		return f.core.items[a] instanceof f.core
				? f.core.items[a].ref
				: f.swfobject.getObjectById(a)
	};
	var m = function(a, b) {
...
(function() {
	...
	d.extend({
		setChartAttribute : function(a) {
			...
			var i = 0, json = this.getChartData(FusionChartsDataFormats.JSON), prop, attList = json.chart
					|| json.graph || {};
			for (prop in a) {
				i += 1;
		...
		getChartAttribute : function(a) {
			var b = (b = this.getChartData(FusionChartsDataFormats.JSON)).chart
					|| b.graph;
			...
			if (typeof a === 'string') {
				...
				d.raiseError(this, '25081429', 'param', '~getChartAttribute()',
...
(function() {
	...
	var z = y.swfobject = function() {
		var w = "undefined", OBJECT = "object", SHOCKWAVE_FLASH = "Shockwave Flash", SHOCKWAVE_FLASH_AX = "ShockwaveFlash.ShockwaveFlash", FLASH_MIME_TYPE = "application/x-shockwave-flash", EXPRESS_INSTALL_ID = "SWFObjectExprInst", ON_READY_STATE_CHANGE = "onreadystatechange", win = window, doc = document, nav = navigator, plugin = false, domLoadFnArr = [main], regObjArr = [], objIdArr = [], listenersArr = [], storedAltContent, storedAltContentId, storedCallbackFn, storedCallbackObj, isDomLoaded = false, isExpressInstallActive = false, dynamicStylesheet, dynamicStylesheetMedia, autoHideShow = true, ua = function() {
			var b = typeof doc.getElementById != w
					&& typeof doc.getElementsByTagName != w
					&& typeof doc.createElement != w, u = nav.userAgent
					.toLowerCase(), p = nav.platform.toLowerCase(), windows = p
					? /win/.test(p)
					: /win/.test(u), mac = p ? /mac/.test(p) : /mac/.test(u), webkit = /webkit/
					.test(u) ? parseFloat(u.replace(
					/^.*webkit\/(\d+(\.\d+)?).*$/, "$1")) : false, ie = !+"\v1", playerVersion = [
					0, 0, 0], d = null;
			if (typeof nav.plugins != w
					&& typeof nav.plugins[SHOCKWAVE_FLASH] == OBJECT) {
				d = nav.plugins[SHOCKWAVE_FLASH].description;
				if (d
						&& !(typeof nav.mimeTypes != w
					...
					playerVersion[0] = parseInt(d.replace(/^(.*)\..*$/, "$1"),
							10);
					playerVersion[1] = parseInt(d.replace(/^.*\.(.*)\s.*$/,
							"$1"), 10);
			...
			if ((typeof doc.readyState != w && doc.readyState == "complete")
					|| (typeof doc.readyState == w && (doc
		...
		function hasPlayerVersion(a) {
			...
			return (b[0] > v[0] || (b[0] == v[0] && b[1] > v[1]) || (b[0] == v[0]
					&& b[1] == v[1] && b[2] >= v[2])) ? true : false
		}
		function createCSS(a, b, c, d) {
			...
			if (!dynamicStylesheet || dynamicStylesheetMedia != m) {
				...
				if (ua.ie && ua.win && typeof doc.styleSheets != w
						&& doc.styleSheets.length > 0) {
					dynamicStylesheet = doc.styleSheets[doc.styleSheets.length
							- 1]
				}
			...
			if (ua.ie && ua.win) {
				...
				if (dynamicStylesheet && typeof doc.createTextNode != w) {
					dynamicStylesheet.appendChild(doc.createTextNode(a + " {"
							+ b + "}"))
				}
			}
		}
		...
		function C(s) {
			var a = /[\\\"<>\.;]/;
			var b = a.exec(s) != null;
			return b && typeof encodeURIComponent != w
	...
	y.core.options.installRedirectMessage = 'You need Adobe Flash '
	...
	var B = /[\\\"<>;]/;
	var C = function(s) {
		return (B.exec(s) !== null)
				&& typeof window.encodeURIComponent !== undefined ? window
	...
	var D = function(e, a) {
		...
		var b = e.sender;
		if (b.ref && typeof b.ref.dataInvokedOnSWF === 'function'
				&& b.ref.dataInvokedOnSWF()
				&& typeof b.ref.getXML === 'function') {
			y.raiseWarning(b, '08300116', 'run',
			...
			return !((b = y.core(a)) instanceof y.core && b.ref && (parent = b.ref.parentNode))
					...
					: {
						width : parent.offsetWidth
								* (c.test(b.width) ? parseInt(b.width, 10)
										/ 100 : 1),
						height : parent.offsetHeight
								* (c.test(b.height) ? parseInt(b.height, 10)
										/ 100 : 1)
					}
		}
	}());
	...
	y.addEventListener('BeforeInitialize', function(a) {
		var b = a.sender;
		...
		if (Boolean(b.options.autoInstallRedirect) === true
				&& !y.swfobject
						.hasFlashPlayerVersion(y.core.options.requiredFlashPlayerVersion
								.toString()) && A === false) {
			A = true;
			if (window.confirm(y.core.options.installRedirectMessage)) {
				window.location.href = 'http://get.adobe.com/flashplayer/'
			}
		}
		if (b.options.dataFormat === undefined
				&& b.options.dataSource === undefined) {
	...
	y.addEventListener('DataLoadRequested', function(a, b) {
		...
		if (window.location.protocol === 'file:'
				&& b.dataFormat === FusionChartsDataFormats.XML
				&& Boolean(c.options.safeMode) === true) {
	...
	y.addEventListener('DataLoadError', function(a, b) {
		...
		if (c.ref && typeof c.ref.showChartMessage === 'function'
	...
	var E = {
		...
		render : function(a, b) {
			...
			if (!this.src) {
				y.raiseError(this, '03102348', 'run', '::FlashRenderer.render',
			...
			c.dataURL = C(dataURL || '');
			y.swfobject.embedSWF(this.src, this.id, this.width, this.height,
					'8.0.0', undefined, c, this.params, this.attributes, b)
		},
		update : function(a) {
			...
			if (a.error === undefined) {
				...
				if (this.isActive() && typeof b.showChartMessage === 'function') {
					...
					this.flashVars.dataXML = '<Invalid'
	...
	E.prototype = {
		getSWFHTML : function() {
			var a = document.createElement('span'), inElm = document
					.createElement('span'), tempId = 'RnVzaW9uQ2hhcnRz'
					+ (new Date()).getTime();
			a.appendChild(inElm);
			...
			y.swfobject
					.embedSWF(this.src, tempId, this.width, this.height,
							'8.0.0', undefined, this.flashVars, this.params,
							this.attrs);
		...
		setTransparent : function(a) {
			...
			this.params.wMode = a === null ? 'window' : (a === true
					? 'transparent'
					: 'opaque')
		},
...
(function() {
	...
	FCC.init = function() {
		var m = function(c, d, e) {
			...
			color : ["AFD8F8", "F6BD0F", "8BBA00", "FF8E46", "008E8E",
			...
			method : {
				...
				exportChart : function(a) {
					...
					if (HCObj.options.exporting.enabled) {
						if (typeof a === 'object') {
							for (var x in a) {
								if (x.toLowerCase() === 'exportformat'
										&& FCFCC.exportFormat[a[x]
												.toLowerCase()]) {
				...
				togglePieSlice : function(a) {
					if (this.ref && this.ref.FCC && this.ref.FCC.series
							&& this.ref.FCC.series[0]
							&& this.ref.FCC.series[0].data
							&& this.ref.FCC.series[0].data[a]
		...
		FCC.prototype = {
			...
			getSWFHTML : noEffect,
			_overrideJSChartConfiguration : function(a) {
				jsConf[this.id] = a
			}
		};
		...
		var o = function(a, b, c, d, e, f) {
			var g, minPowerOfTen, powerOfTen, y_interval, rangePowerOfTen, rangeInterval, y_topBound, y_lowerBound, yMaxGiven, yMinGiven, yMax, yMin, range, interval;
			...
			if (b < 0) {
				y_lowerBound = -1
						* ((Math.floor(Math.abs(b / y_interval)) + 1) * y_interval)
			} else {
				if (f) {
					y_lowerBound = 0
				} else {
					y_lowerBound = Math.floor(Math.abs(b / y_interval) - 1)
							* y_interval;
		...
		function createMsg(a, b, c) {
			...
			d.title.y = parseInt(document.getElementById(a).offsetHeight, 10)
					/ 2;
			d.chart.renderTo = a;
		...
		function drawOverlayButton(b) {
			if (b.show) {
				...
				c.style.border = '1px solid #'
						+ (b.borderColor ? b.borderColor : "7f8975");
				c.style.backgroundColor = '#'
						+ (b.bgColor ? b.bgColor : "edefec");
				...
				c.style.fontWeight = parseInt(b.bold, 10) === 0
						? 'normal'
						: 'bold';
		...
		function convertColor(a, b, c) {
			...
			if (c && c.match(/^rgba/ig)) {
				...
				B = tempArr[2];
				if (!b) {
					b = parseInt(tempArr[3].slice(0, tempArr[3].indexOf(')')),
							10)
							* 100
				}
			}
			if (a) {
				...
				switch (colorStr.length) {
					case 3 :
						colorStr = colorStr[0] + colorStr[0] + colorStr[1]
								+ colorStr[1] + colorStr[2] + colorStr[2];
						break;
					case 6 :
						break;
					default :
		...
		var v = function(a, b, d) {
			...
			a = e.indecimalseparator !== '' ? a.toString().replace(
					e.indecimalseparator, '.') : a;
			a = e.inthousandseparator !== '' ? a.toString().replace(
					e.inthousandseparator, '') : a;
			if (d !== 1) {
			...
			if (e.formatnumber == 1) {
				...
				result = (splitVal === undefined) || (decimalVal === 0)
						? formatedNum
						: formatedNum + decimalVal
			}
			...
			if (e.forcedecimals == 1) {
				...
				if (decimalLength > e.decimals) {
					if (e.decimals == 1 && decimalVal[0] >= 5) {
						last = result[result.substring(0, result.indexOf('.')).length
								- 1];
						last += 1;
						...
						last = decimalVal[e.decimals] >= 5
								? (parseInt(decimalVal[e.decimals - 1], 10) + 1)
								: decimalVal[e.decimals - 1];
						decimalVal = decimalVal.substring(0, e.decimals - 1)
								+ last;
			...
			if ((a === 2 && this.link) || (a === 1 && this.options.chart.link)) {
				...
				switch (arr[0].toLowerCase()) {
					...
					case 'p' :
						...
						break;
					case 'newchart' :
						var d = arr[1].toLowerCase().search('url') != -1
								? 'URL'
								: FusionChartsDataFormats.JSON, data, x, chartJson, linkchartId;
						if (d === 'URL') {
							data = c.replace(/[^\-]+-[^\-]+-/i, '')
						} else {
		...
		var F = function(a, b) {
			...
			if (!FCFCC.highCharts[chart]) {
				HCJson = createMsg(id, 'ChartNotSupported',
						'featurenotsupported')
			} else if (n[id].msgTxt) {
				...
				HCJson = covertToFCC(id, chart, jsonObj, id,
		...
		var J = function() {
			...
			for (var x in FCC.items) {
				...
				if (FCCContainer
						&& (FCCContainer.offsetWidth != a.FCCObj.FCC.chartWidth || FCCContainer.offsetHeight != a.FCCObj.FCC.chartHeight)) {
					I(a, id)
				}
			}
		};
			...
			return {
							...
							formatter : function() {
								...
								if (this.series.type == 'pie') {
									...
									y = showvalue === '1'
											? (Math
													.round(this.percentage
															* 100) / 100)
													+ (a.chart.showpercentagevalues === '1'
															? '%'
															: '')
											: '';
									return parseStr(x
											+ (x !== '' && y !== ''
													? a.chart.tooltipsepchar
													: '') + y)
								} else if (showvalue === '1') {
									if (this.point.options.displayvalue) {
										...
										if (this.series.options.FCtype == 1.5
				...
				tooltip : {
					...
					formatter : function() {
						...
						if (this.point.options.tooltext) {
							...
							if (this.series.options.FCtype >= 2
									&& this.series.name !== ' ') {
							...
							if (this.series.options.FCtype == 1.5
								...
								x = (this.point.FCname ? this.point.FCname
										+ charSep : '')
										+ (Math.round(this.percentage * 100) / 100)
										+ (a.chart.showpercentintooltip === '0'
												? ''
												: '%')
				...
				xAxis : {
					labels : {
						...
						style : {},
						formatter : function() {
							return parseStr(typeof this.value === 'string'
									? this.value
									: '')
						},
						align : 'right'
		...
		function addVline(a, b, c, d, e) {
			a.xAxis.plotLines.push({
				color : convertColor(b || '#444444', c || 100),
				width : d ? d : 1,
				value : e
			})
		}
		function addPoint(a, b, c, d, e, f, g, h, i) {
			var x, y, isAbs = FCFCC.valueAbs[d], value, seriesType = a.type
					|| f.chart.defaultSeriesType, dataseperator, tempY, dataYmax, dataYmin, loopMax;
			if (!a.yAxis) {
				dataYmax = 'dataY0max';
				dataYmin = 'dataY0min'
			} else {
				...
				for (y = 0; y < loopMax; y += 1) {
					if (typeof b[y] === 'object') {
						if (!b[y].vline) {
							...
							tempY = (isAbs ? Math.abs(parseFloat(b[y].value
									|| b[y].y)) : parseFloat(b[y].value
									|| b[y].y));
							if (tempY < 0) {
								f.FCconf.negative = true
							}
							if (typeof f.FCconf[dataYmax] === 'undefined'
									|| tempY > f.FCconf[dataYmax]) {
								f.FCconf[dataYmax] = tempY
							}
							if (typeof f.FCconf[dataYmin] === 'undefined'
									|| tempY < f.FCconf[dataYmin]) {
							...
							a.data.push({
								y : tempY || 0,
								x : (isAbs
										? Math.abs(parseFloat(b[y].x))
										: parseFloat(b[y].x))
										|| undefined,
								z : (isAbs
										? Math.abs(parseFloat(b[y].z))
										: parseFloat(b[y].z))
										|| undefined,
								color : (c < 2 || b[y].color || b[y].alpha)
										? convertColor(b[y].color
												|| (c < 2 && g(y)), b[y].alpha,
												a.color)
										: undefined,
								...
								marker : (seriesType === 'areaspline'
										|| seriesType === 'spline'
										|| seriesType === 'area' || seriesType === 'line')
										? {
											enabled : b[y].drawanchors
													? (b[y].drawanchors == '1'
															? true
															: false)
													: undefined,
											radius : b[y].anchorradius
													? b[y].anchorradius
													: undefined,
											symbol : w(parseInt(
													b[y].anchorsides, 10)),
											fillColor : b[y].anchorbgcolor
													? convertColor(
															b[y].anchorbgcolor
																	|| 'FFFFFF',
															b[y].anchorbgalpha
																	|| 100)
													: undefined,
											lineColor : b[y].anchorbordercolor
													? convertColor(
															b[y].anchorbordercolor
																	|| '767575',
															b[y].anchoralpha
																	|| 100)
													: undefined,
											lineWidth : b[y].anchorborderthickness
													? b[y].anchorborderthickness
													: undefined
										}
										: (c === 7.5 ? {
											radius : b[y].z ? Math
							...
							if (c < 2) {
								f.xAxis.categories.push(b[y].showlabel === '0'
										? ''
										: (b[y].label || ''))
							}
						} else if (c < 2) {
							addVline(f, b[y].color, b[y].alpha, b[y].thickness,
									y - 0.5)
						}
					} else if (typeof b[y] === 'string'
							&& i.chart.compactdatamode == '1') {
						dataseperator = i.chart.dataseparator || '|';
						value = b[y].split(dataseperator);
						for (x = 0; x < value.length; x += 1) {
							tempY = isAbs
									? Math.abs(parseFloat(value[x]))
									: parseFloat(value[x]);
							if (tempY < 0) {
								f.FCconf.negative = true
							}
				...
				for (x = 0; x < a.categories[0].category.length; x += 1) {
					if (typeof a.categories[0].category[x] === 'object') {
						if (!a.categories[0].category[x].vline) {
							if (b == 2.5) {
								zoomCat.push(a.categories[0].category[x].label
										? a.categories[0].category[x].label
										: '')
							} else {
								c.xAxis.categories
										.push(a.categories[0].category[x].label
												? a.categories[0].category[x].label
												: '')
							}
							countCat += 1
						} else {
							addVline(c, a.categories[0].category[x].color,
									a.categories[0].category[x].alpha,
									a.categories[0].category[x].thickness,
									countCat - 0.5)
						}
					} else if (typeof a.categories[0].category[x] === 'string'
							&& a.chart.compactdatamode == '1') {
						dataseperator = a.chart.dataseparator
								? a.chart.dataseparator
								: '|';
						if (b == 2.5) {
							zoomCat = zoomCat
		...
		function MSStacked(b, c, d, e, f, g, h, i) {
			var x, y, z, newSeries, count_set = 0, max_cat = 0, a = [], plotWidth = f
					- (e.chart.margin[1] + e.chart.margin[3]), plotHight = g
					- (e.chart.margin[0] + e.chart.margin[2]), noStack, pointWidth, catWidth, tempheight, maxHight = 0, pointValue = 0;
			...
			if (b.dataset && b.dataset instanceof Array) {
				...
				for (x = 0; x < b.dataset.length; x += 1) {
					tempheight = [];
					if (b.dataset[x].dataset
							&& b.dataset[x].dataset instanceof Array) {
						for (y = 0; y < b.dataset[x].dataset.length; y += 1) {
							newSeries = {
								FCtype : c,
								type : b.dataset[x].dataset[y].renderas
										? FCFCC.combi[b.dataset[x].dataset[y].renderas
												.toLowerCase()]
										: undefined,
								name : b.dataset[x].dataset[y].seriesname
										? b.dataset[x].dataset[y].seriesname
										: ' ',
								data : [],
								color : convertColor(b.dataset[x].color
										|| i(count_set),
										b.dataset[x].dataset[y].alpha || 100),
								yAxis : b.dataset[x].dataset[y].parentyaxis == 'S'
										? 1
										: undefined,
							...
							if (b.dataset[x].dataset[y].data instanceof Array) {
								for (z = 0; z < max_cat; z += 1) {
									tempheight[z] = tempheight[z]
											? tempheight[z]
											: 0;
									pointValue = (b.dataset[x].dataset[y].data[z] && b.dataset[x].dataset[y].data[z].value)
											? parseFloat(b.dataset[x].dataset[y].data[z].value)
											: '';
									tempheight[z] += pointValue
											? pointValue
											: 0;
									newSeries.data.push({
										MWidth : pointWidth,
										...
										id : h,
										color : b.dataset[x].dataset[y].data[z].color
												? convertColor(
														b.dataset[x].dataset[y].data[z].color
																|| i(count_set),
														b.dataset[x].dataset[y].data[z].alpha
																|| 100)
												: undefined
									})
								}
		...
		function addData(c, d, e, f, g, h, i) {
			...
			getScolor = function(a) {
				var b;
				b = (c.chart.palettecolors)
						? ((typeof c.chart.palettecolors === 'string')
								? c.chart.palettecolors.split(',')
								: c.chart.palettecolors)
						: FCFCC.color;
				return b[a % b.length]
			};
			switch (d) {
				...
				case 1.5 :
					...
					if (c.data) {
						...
						a.push(z);
						if ((e == 'Pareto2D' || e == 'Pareto3D')
								&& c.chart.showcumulativeline !== '0') {
							pareto = t(z.data);
							a.push(pareto)
						}
					}
				...
				case 6.5 :
					switch (d) {
						...
						case 3 :
							...
							break;
						case 5.5 :
							f.plotOptions.column.stacking = c.chart.stack100percent == '1'
									? 'percent'
									: 'normal'
					}
					addCategory(c, d, f);
					if (d !== 6) {
						if (c.dataset) {
							...
							for (x = 0; x < c.dataset.length; x += 1) {
								z = createSeries(
										d,
										convertColor(c.dataset[x].color
												|| getScolor(count_set),
												c.dataset[x].alpha || 100),
										c.dataset[x].renderas
												? FCFCC.combi[c.dataset[x].renderas
														.toLowerCase()]
												: ((c.dataset[x].parentyaxis == 'S' && (d == 5 || d == 5.5))
														? 'line'
														: undefined),
										c.dataset[x].seriesname,
										c.dataset[x].parentyaxis == 'S',
										c.dataset[x].showvalues,
										c.dataset[x].plotbordercolor
												? convertColor(
														c.dataset[x].plotbordercolor,
														c.dataset[x].plotborderalpha
																|| 95)
												: undefined,
										parseInt(
												c.dataset[x].plotborderthickness
														|| c.dataset[x].linethickness,
												10)
												|| undefined);
								var j = z.type || f.chart.defaultSeriesType;
								z.marker = (j === 'area' || j === 'line'
										|| j === 'spline' || j === 'areaspline')
										? {
											enabled : c.dataset[x].drawanchors
													? (c.dataset[x].drawanchors == '1'
															? true
															: false)
													: undefined,
											radius : c.dataset[x].anchorradius
													? c.dataset[x].anchorradius
													: undefined,
											symbol : w(parseInt(
													c.dataset[x].anchorsides,
													10)),
											fillColor : c.dataset[x].anchorbgcolor
													? convertColor(
															c.dataset[x].anchorbgcolor
																	|| 'FFFFFF',
															c.dataset[x].anchorbgalpha
																	|| 100)
													: undefined,
											lineColor : c.dataset[x].anchorbordercolor
													? convertColor(
															c.dataset[x].anchorbordercolor
																	|| '767575',
															c.dataset[x].anchoralpha
																	|| 100)
													: undefined,
											lineWidth : c.dataset[x].anchorborderthickness
													? c.dataset[x].anchorborderthickness
													: undefined
										}
										: null;
								count_set += 1;
								if (c.dataset[x].data) {
									addPoint(z, c.dataset[x].data, d, e, i, f,
				...
				case 7.5 :
					...
					if (c.categories && c.categories[0]
						...
						for (x = 0; x < c.categories[0].category.length; x += 1) {
							if (c.categories[0].category[x].showverticalline
									&& c.categories[0].category[x].showverticalline == '1') {
								addVline(f, c.categories[0].verticallinecolor,
										c.categories[0].verticallinealpha,
										c.categories[0].verticallinethickness,
										c.categories[0].category[x].x)
							}
							f.xAxis.categories[c.categories[0].category[x].x] = c.categories[0].category[x].label
									? c.categories[0].category[x].label
									: ''
						}
					}
					...
					if (c.dataset) {
						...
						for (x = 0; x < c.dataset.length; x += 1) {
							z = createSeries(
									...
									c.dataset[x].seriesname,
									c.dataset[x].parentyaxis == 'S',
									c.dataset[x].showvalues, undefined,
									undefined);
							z.marker = {
								symbol : (d == 7.5) ? 'circle' : w(parseInt(
										c.dataset[x].anchorsides, 10)),
								states : {
									hover : {
										enabled : false
									}
					...
					f.xAxis.max = c.chart.xaxismaxvalue ? parseInt(
							c.chart.xaxismaxvalue, 10) : undefined;
					f.xAxis.min = c.chart.xaxisminvalue ? parseInt(
							c.chart.xaxisminvalue, 10) : undefined;
					f.xAxis.showLastLabel = true;
					break;
				default :
					break
			}
			if (!((d === 3 || d === 5.5) && c.chart.stack100percent === '1')
					&& e !== 'Marimekko') {
				if (d === 3 || d === 5.5) {
				...
				if (d === 1.5 || d === 5.5 || d === 5 || d === 6.5) {
					yCalTemp = o(f.FCconf.dataY0max, f.FCconf.dataY0min,
							c.chart.pyaxismaxvalue, c.chart.pyaxisminvalue, '',
							c.chart.setadaptiveymin !== '1');
					f.yAxis[0].max = yCalTemp.Max;
					f.yAxis[0].min = yCalTemp.Min;
					if (d !== 1.5) {
						yCalTemp = o(f.FCconf.dataY1max, f.FCconf.dataY1min,
								c.chart.syaxismaxvalue, c.chart.syaxisminvalue,
								'', c.chart.setadaptivesymin !== '1');
						f.yAxis[1].max = yCalTemp.Max;
						f.yAxis[1].min = yCalTemp.Min
					}
				} else {
					yCalTemp = o(f.FCconf.dataY0max, f.FCconf.dataY0min,
							c.chart.yaxismaxvalue, c.chart.yaxisminvalue, '',
							c.chart.setadaptiveymin !== '1');
					f.yAxis[0].max = yCalTemp.Max;
					f.yAxis[0].min = yCalTemp.Min
				}
		...
		function ModyfyLegend(b, c) {
			...
			if (c.chart.showlegend == '0') {
				...
				b.legend.backgroundColor = convertColor(c.chart.legendbgcolor
						|| '#ffffff', c.chart.legendbgalpha || 100);
				b.legend.borderColor = convertColor(c.chart.legendbordercolor
						|| '#545454', c.chart.legendborderalpha || 100);
		...
		function convertMarimeko(a, b, c, d, e) {
			var f = c - (a.chart.margin[1] + a.chart.margin[3]), plotHight = d
					- (a.chart.margin[0] + a.chart.margin[2]), y, z, arr = [], total = 0, temp, setPersent = false, catCountFlag = true, temp2, totalpercent = 0;
			if (a.xAxis.labels.enabled === false
					&& b.chart.showxaxispercentvalues != '0') {
			...
			for (y = 0; y < a.series.length; y += 1) {
				...
				for (z = 0; z < a.series[y].data.length; z += 1) {
					...
					if (catCountFlag) {
						if (a.xAxis.labels.enabled !== false) {
							a.labels.items.push(createText(
									a.xAxis.categories[z], plotHight, temp))
						}
						if (b.chart.showsum != '0') {
							a.labels.items.push(createText(v(arr[z], b.chart),
									'-14', temp))
						}
					}
					temp2 = setPersent
							? parseFloat(e[0].category[z].widthpercent) / 100
							: (arr[z] / total);
					totalpercent += temp2;
					temp += a.series[y].data[z].MWidth = Math.round(f * temp2);
					if (catCountFlag && b.chart.showxaxispercentvalues != '0'
							&& totalpercent < 0.9) {
						a.labels.items.push(createText((Math.round(totalpercent
								* 10000) / 100)
								+ '%', plotHight - 15, temp - 20))
					}
				}
				catCountFlag = false
			}
		...
		function covertToFCC(a, b, c, d, e, f) {
			...
			if (g.chart.defaultSeriesType === 'line'
					|| g.chart.defaultSeriesType === 'spline'
					|| g.chart.defaultSeriesType === 'area'
					|| g.chart.defaultSeriesType === 'areaspline') {
			...
			g.plotOptions.series.animation = c.chart.animation === '0'
					? false
					: true;
			...
			if (c.chart.showyaxisvalues == '0'
					|| c.chart.showdivlinevalues == '0'
					|| c.chart.showdivlinevalue == '0') {
				g.yAxis[0].labels.enabled = false;
				g.chart.margin[3] = 15
			}
			if (((y == 5 || y == 5.5) && c.chart.showyaxisvalues != '0' && c.chart.showdivlinesecondaryvalue != '0')
					|| (y === 1.5 && c.chart.showdivlinesecondaryvalue != '0')) {
				g.yAxis[1].labels.enabled = true;
				g.chart.margin[1] = 50
			}
			g.xAxis.labels.style = {
				fontFamily : c.chart.outcnvbasefont || c.chart.basefont
						|| 'Verdana',
				fontSize : (c.chart.outcnvbasefontsize || c.chart.basefontsize || '10')
						+ 'px',
				color : c.chart.outcnvbasefontcolor || c.chart.basefontcolor
						|| '#555555'
			};
			...
			g.plotOptions.line.marker.lineColor = g.plotOptions.areaspline.marker.lineColor = g.plotOptions.spline.marker.lineColor = convertColor(
					c.chart.anchorbordercolor || '767575', c.chart.anchoralpha
							|| 100);
			g.plotOptions.area.marker.lineColor = convertColor(
					c.chart.anchorbordercolor || '767575', c.chart.anchoralpha
							|| 0);
			...
			g.plotOptions.line.marker.fillColor = g.plotOptions.spline.marker.fillColor = convertColor(
					c.chart.anchorbgcolor || 'FFFFFF', c.chart.anchorbgalpha
							|| 100);
			g.plotOptions.area.marker.fillColor = g.plotOptions.areaspline.marker.fillColor = convertColor(
					c.chart.anchorbgcolor || 'FFFFFF', c.chart.anchorbgalpha
							|| 0);
			if (c.chart.anchorsides) {
				g.plotOptions.area.marker.symbol = g.plotOptions.line.marker.symbol = g.plotOptions.areaspline.marker.symbol = g.plotOptions.spline.marker.symbol = w(parseInt(
						c.chart.anchorsides, 10))
			}
			g.plotOptions.line.lineWidth = g.plotOptions.spline.lineWidth = parseInt(
					c.chart.linethickness, 10)
					|| (y === 1.5 ? 2 : 3);
			g.plotOptions.line.color = g.plotOptions.spline.color = convertColor(
					c.chart.linecolor || '767575', c.chart.linealpha || 100);
			g.plotOptions.area.color = g.plotOptions.areaspline.color = convertColor(
					c.chart.plotfillcolor || '767575', c.chart.plotfillalpha
							|| 90);
			g.plotOptions.area.lineColor = g.plotOptions.areaspline.lineColor = convertColor(
					c.chart.plotbordercolor || "333333",
					c.chart.plotborderalpha || 95);
			g.plotOptions.area.lineWidth = g.plotOptions.areaspline.lineWidth = parseInt(
					c.chart.plotborderthickness, 10)
					|| 1;
			g.yAxis[0].alternateGridColor = convertColor(
					c.chart.alternatehgridcolor || 'EEEEEE',
					c.chart.alternatehgridalpha || 50);
			g.yAxis[0].gridLineColor = convertColor(c.chart.divlinecolor
					|| '717170', c.chart.divlinealpha || 40);
			g.yAxis[0].gridLineWidth = c.chart.divlinethickness
					? c.chart.divlinethickness
					: 1;
			if (c.chart.caption) {
				...
				g.title.style = {
					...
					fontSize : '13px',
					'font-weight' : 'bold',
					color : c.chart.outcnvbasefontcolor
							|| c.chart.basefontcolor || '#555555'
				}
			}
			...
			if (c.chart.xaxisname) {
				...
				g.xAxis.title.style = {
					fontFamily : c.chart.outcnvbasefont || c.chart.basefont
							|| 'Verdana',
					fontSize : (c.chart.outcnvbasefontsize
							|| c.chart.basefontsize || '10')
							+ 'px',
					'font-weight' : 'bold',
					color : c.chart.outcnvbasefontcolor
							|| c.chart.basefontcolor || '#555555'
			...
			g.tooltip.backgroundColor = convertColor(c.chart.tooltipbgcolor
					|| 'FFFFFF', c.chart.tooltipbgalpha || 100);
			g.tooltip.borderColor = convertColor(c.chart.tooltipbordercolor
					|| '545454', c.chart.tooltipborderalpha || 100);
					...
					if (c.trendlines[x].line) {
						for (z = 0; z < c.trendlines[x].line.length; z += 1) {
							if (c.trendlines[x].line[z].istrendzone == '1') {
								g.yAxis[0].plotBands.push({
									color : convertColor(
											c.trendlines[x].line[z].color
													|| '333333',
											c.trendlines[x].line[z].alpha || 99),
									from : c.trendlines[x].line[z].startvalue
											? c.trendlines[x].line[z].startvalue
											: 0,
							...
							g.yAxis[0].plotLines.push({
								color : convertColor(
										c.trendlines[x].line[z].color
												|| '333333',
										c.trendlines[x].line[z].alpha || 99),
								value : c.trendlines[x].line[z].startvalue
										? c.trendlines[x].line[z].startvalue
										: 0,
								width : c.trendlines[x].line[z].thickness
										? c.trendlines[x].line[z].thickness
										: 1
							})
						}
					}
					...
					if (c.vtrendlines[x].line) {
						for (z = 0; z < c.vtrendlines[x].line.length; z += 1) {
							if (c.vtrendlines[x].line[z].istrendzone !== '0') {
								g.xAxis.plotBands.push({
									color : convertColor(
											c.vtrendlines[x].line[z].color
													|| '333333',
											c.vtrendlines[x].line[z].alpha
													|| 99),
									from : c.vtrendlines[x].line[z].startvalue
											? c.vtrendlines[x].line[z].startvalue
											: 0,
									to : c.vtrendlines[x].line[z].endvalue
								})
							}
							g.xAxis.plotLines.push({
								color : convertColor(
										c.vtrendlines[x].line[z].color
												|| '333333',
										c.vtrendlines[x].line[z].alpha || 99),
								value : c.vtrendlines[x].line[z].startvalue
										? c.vtrendlines[x].line[z].startvalue
										: 0,
								width : c.vtrendlines[x].line[z].thickness
										? c.vtrendlines[x].line[z].thickness
										: 1
							})
						}
					}
			...
			g.exporting.buttons.exportButton.enabled = c.chart.exportshowmenuitem == '0'
					? false
					: true;
			g.exporting.filename = c.chart.exportfilename
					? c.chart.exportfilename
					: 'FusionCharts';
			g.exporting.width = e;
			if (c.styles && c.styles.definition instanceof Array
					&& c.styles.application instanceof Array) {
				...
				for (j = 0; j < c.styles.application.length; j += 1) {
					...
					for (l = 0; l < styleArr.length; l += 1) {
						...
						if (tempstyle[styleName]) {
							FCFCC.supportedStyle[tempstyle[styleName].type](g,
									c.styles.application[j].toobject
											.toLowerCase(),
									tempstyle[styleName])
						}
					}
			...
			g.chart.borderWidth = c.chart.showborder == '0'
					? 0
					: (c.chart.borderthickness ? c.chart.borderthickness : 1);
			g.chart.borderColor = convertColor(c.chart.bordercolor || '767575',
					c.chart.borderalpha || 50);
			g.chart.plotBorderColor = convertColor(c.chart.canvasbordercolor
					|| '545454', c.chart.canvasborderalpha || 100);
			g.chart.plotBorderWidth = c.chart.canvasborderthickness
					? c.chart.canvasborderthickness
					: 2;
			g.plotOptions.series.borderColor = convertColor(
					c.chart.plotbordercolor || '333333',
					c.chart.plotborderalpha || 95);
			g.plotOptions.series.borderWidth = c.chart.showplotborder == '0'
					? 0
					: (c.chart.plotborderthickness
							? c.chart.plotborderthickness
							: 1);
			g.plotOptions.series.borderRadius = c.chart.useroundedges == '1'
					? 5
					: 0;
			g.yAxis[0].plotLines.push({
				color : convertColor(c.chart.zeroplanecolor || '717170',
						c.chart.zeroplanealpha || 80),
				value : 0,
			...
			g.chart.backgroundColor = r(0, 0, e, f ? f : 400,
					c.chart.bgangle !== undefined ? c.chart.bgangle : 270,
					c.chart.bgcolor ? c.chart.bgcolor : "CBCBCB,E9E9E9",
					c.chart.bgalpha ? c.chart.bgalpha : "50,50",
					c.chart.bgratio ? c.chart.bgratio : "0,100");
			var h, plotY;
			if ('\v' === 'v') {
				h = 0;
				plotY = 0
			...
			g.chart.plotBackgroundColor = (s(b) == 'pie')
					? 'rgba(255,255,255,0)'
					: (r(h, plotY, e - (g.chart.margin[1] + g.chart.margin[3]),
							(f ? f : 400)
									- (g.chart.margin[0] + g.chart.margin[2]),
							c.chart.canvasbgangle !== undefined
									? c.chart.canvasbgangle
									: 0, c.chart.canvasbgcolor
									? c.chart.canvasbgcolor
									: "FFFFFF", c.chart.canvasbgalpha
									? c.chart.canvasbgalpha
									: "100", c.chart.canvasbgratio
									? c.chart.canvasbgratio
									: ""));
			return g
		}
		FCC.dataFormat = 'json';
...
(function() {
	...
	var l = (function() {
			...
			child : function(a, b, c, d) {
				...
				for (i = 0; i < b.length; i += 1) {
					...
					if (b[i].nodeType === f) {
						...
						if (typeof e.tag[nodeName] === 'object'
								&& typeof e.tag[nodeName][c] === 'string') {
					...
					} else if (b[i].nodeType === XML_TEXTNODE && d.chart
							&& parseInt(d.chart.compactdatamode, 10)
							&& typeof e.text[c] === 'string') {
						nodeName = e.text[c];
						childObj = b[i].data;
						g.append(childObj, a, nodeName)
					}
		...
		var h = function(a) {
			...
			a = a.toString().replace(/<\!--[\s\S]*?-->/g, '').replace(
					/<\?xml[\s\S]*?\?>/ig, '').replace(/&(?!([^;\n\r]+?;))/g,
					'&amp;$1');
			a = fastTrim(a);
			if (!a) {
			...
			if (!(xmlDoc.childNodes.length === 1
					...
					&& (rootName = root.nodeName.toLowerCase()) && (rootName === 'chart'
	...
	var m = (function() {
		var d = {
			...
			qualify : function(a, b, c) {
				c = c.toLowerCase();
				return typeof this.items[a][c] === 'object'
						? this.items[a][c][b.toLowerCase()]
						: this.items[a][c]
			}
		};
		var f = function(a, b) {
			...
			if (a instanceof Array) {
				...
				for (item in a) {
					if (a[item] instanceof Array
							&& (qualifier = d.qualify('group', item, b))) {
						innerXML += '<' + item + '>' + f(a[item], qualifier)
								+ '</' + item + '>'
...
(function() {
	...
	h.prototype.configuration = function() {
		var a = g[this.root.id][this.level]
				|| (g[this.root.id][this.level] = {});
	...
	f.extend({
		configureLink : function(a, b) {
			if (a instanceof Array) {
				...
				f
						.raiseError(this, '25081731', 'param',
								'~configureLink()',
	...
	f.addEventListener('LinkedChartInvoked', function(a, b) {
		var c = a.sender, param = c.clone({
			dataSource : b.data,
			dataFormat : b.linkType === 'URL'
					? FusionChartsDataFormats.XMLURL
					: b.linkType,
		...
		if (f.core.items[param.id] instanceof f.core
				&& param.strictLinkId === true) {
	...
	f.addEventListener('OverlayButtonClick', function(a) {
		...
		if (!parent.isActive()
				&& b.options.containerElement === parent.options.containerElement
				&& b.options.insertMode === 'replace') {
			parent.render()
		}
	});
	f.addEventListener('Loaded', function(a) {
		...
		if (!(b.ref && typeof b.ref.drawOverlayButton === 'function')) {
			f.raiseWarning(b, '04091602', 'run', '::LinkManager^Loaded',
					'Unable to draw overlay button on object. -' + b.id);
			return
		}
		var c = f.extend({
			show : true
...
(function() {
	...
	var l = {
		...
		removeCanvasElementOf : function(a) {
			if (a.__fusioncharts__canvascreated !== true || !a.parentNode
					|| a.parentNode === null) {
				return
			}
		...
		drawRLE : function(a, b, c, d, e) {
			...
			if (typeof f.putImageData === 'function'
					&& typeof f.createImageData === 'function') {
		...
		drawText : function(a, b, c, d) {
			...
			if (typeof e.fillText === 'function') {
				...
				j.raiseWarning(j.core, '25081803', 'run',
		...
		invoke : function(a) {
			...
			if (typeof a !== 'undefined') {
				if (this.cssNode !== undefined
						&& this.cssNode.parentNode !== undefined) {
	...
	var n = function(a) {
		...
		if (b === undefined || typeof b.prepareImageDataStream !== 'function'
				|| b.prepareImageDataStream() === false) {
		...
		try {
			...
			m.invoke('error');
			j
					.raiseError(a.sender, '25081807', 'run',
		...
		try {
			if (l.drawRLE(
					l.getCanvasElementOf(a.sender.ref, b.width, b.height),
					b.stream, b.width, b.height, b.bgColor) === true) {
				if (activeItems[a.sender.id]) {
			...
			m.invoke('error');
			j.raiseError(a.sender, '25081810', 'run',
	...
	j.extend({
		printManager : {
			...
			enabled : function(a) {
				...
				if ('\v' === 'v'
						|| j.renderer.currentRendererName() !== 'flash'
					...
					j.raiseWarning(j.core, '25081816', 'run',
			...
			managedPrint : function() {
				...
				if (!j.core.printManager.isReady()) {
					...
					j.addEventListener('PrintReadyStateChange',
							j.core.printManager.managedPrint);
					return
				}
				if (typeof arguments[1] === 'object'
						&& arguments[1].ready !== true) {
					return
				}
				j.removeEventListener('PrintReadyStateChange',
						j.core.printManager.managedPrint);
				window.print()
			}
		}
