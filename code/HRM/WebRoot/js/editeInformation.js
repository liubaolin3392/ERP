				...
				if (storeId == "1") {
					if (confirm("您确定提交吗？")) {
						...
						if (phone.search("^-?\\d+$") != 0) {
							...
							$("#phone1"+ tempId).css("background", "#B7E1FA");
							$("INPUT").keypress(function() {
								$("#phone1"+ tempId).css("background", "#FFFFFF");
							});
							return false;
						}
						...
						$("#userLevel" + tempId).html(userLevel);
...
function submitEdit(userId, userName, password, age, department, sex, position,
	...
	$.post("president_updateEmployee.action",{"employee.userName":userName,"employee.password":password,"employee.sex":sex,"employee.department":department,"employee.position":position
...
function submitdelete(eid) {
	$.post("president_deleteEmployee.action",{"employee.eid":eid},function(data){
